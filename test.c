...
{
    static struct etimer et;    /* Timer needed to awake the process */
    ...
    while (1) {
        /* The process is awaken every second */
...
static void process_routing(struct broadcast_conn *c, const linkaddr_t *from)
{
    ...
    if (rank == 0 || p.rank + 1 < rank) { 
        /* The node has no parent or the sender needs less hops to reach the root than the current parent */
        /* Choose the sender as the new parent */
        rank = p.rank + 1;
...
static void process_cmd(struct broadcast_conn *c, const linkaddr_t *from)
{
    ...
    switch (p.command_type) { /* Process command */
        ...
        case MUTE_SUBJECT:
            ...
            } else if (p.command_value_extra != my_id[0] 
                       && from->u8[0] == parent_id[0] 
                       && from->u8[1] == parent_id[1]) 
            {
                changed = 1;
            }
            break;
